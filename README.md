...
Prereqs: Node.js 18+, a git repo you want to change (JS or Python), and a logged-in Claude Code or `ANTHROPIC_API_KEY`. Runs spend real Anthropic tokens — a small spec costs a few dollars, larger ones more; `cc-orch usage` reports cost after every run.
...
The package installs two equivalent commands, `nightfoundry` and `cc-orch` — they point at the same CLI. Every example below uses `cc-orch` (its long-standing name); pick whichever you prefer.
...
cc-orch plans, executes, verifies, and shows the diff. Nothing lands without passing the JS-side gates first.
...
cc-orch run my-feature.uspec.json -a    # hand-written uspecs run directly (dry-run queues .md specs only)
...
```bash
cc-orch dry-run <spec>       # cheap safety check: plan + assumption-check, no execution
cc-orch run <spec>           # interactive: confirms the plan, shows each diff
cc-orch run <spec> -a        # auto-approve (headless)
cc-orch resume --batch -a    # work through the dry-run queue unattended
```
...
| **Final `test:all` gate** | Blocks archival if the full suite isn't green (override: `cc-orch archive --skip-test-gate`). |
...
| **Plan-scope lint** | Every task declares the files it will touch; unmatched scope items either fail or warn (per [`--allow-incomplete-scope`](./.claude/skills/cc-orch-operator/references/commands.md)). | `src/orchestrator/gates/plan-scope-lint.js` |
...
| **Final `test:all` gate** | Blocks archival if the full suite isn't green (override: `cc-orch archive --skip-test-gate`). | `src/cli/commands/archive.js` |
...
- **Interrupt-safe.** Ctrl-C, API outage, machine sleep → state is saved. `cc-orch resume` continues where it left off; completed work stays intact.
- **Batch.** `cc-orch dry-run` queues a validated spec. `cc-orch resume --batch -a` works the queue unattended: each spec runs in isolation, a failed spec is reverted, and the queue continues (`cc-orch queue list`).
- **Park.** When a run hits something only a human can decide (a failed spec assumption, an analyzer escalation, a rejected review), the entry is **parked**, not lost. `cc-orch park list` shows what's waiting and why; `cc-orch park resolve <slug> --requeue|--waive|--reject` sends it back with your decision.
- **Non-blocking warnings.** Reviewer findings that don't warrant halting a run land in a ledger (`cc-orch warnings list`). Batch several into one fix spec with `cc-orch warnings brainstorm <ids>` when you're ready.
- **Archives.** Every completed run is archived under `archives/<id>/` with the full state, verifier verdicts, reviewer findings, cost breakdown, and an HTML report (`cc-orch archive show <id> --report`). Diffable across runs (`cc-orch archive diff a b`, `cc-orch dispersion`).
...
- Cost transparency: per-run and cross-archive (`cc-orch usage --all`), per-role breakdowns.
- A local web dashboard (`cc-orch ui`) for watching runs.
...
```bash
cc-orch task "..."                              # one-off change, no spec
cc-orch brainstorm "..."                        # prose → spec pair (interactive)
cc-orch dry-run <spec.md>                       # plan + assumption-check, no execution; queues for batch
cc-orch run <spec.md | spec.uspec.json> [-a]    # run a spec (also: --spec-stdin to pipe a uspec in)
cc-orch status [<mission-id>]                   # progress
```

**Batch & recovery**

```bash
cc-orch resume [--batch] [-a]                   # resume an interrupted run / work through the queue
cc-orch queue list | remove <slug>              # batch queue
...
cc-orch dispersion [<id> | compare <a> <b>]     # archive fingerprints
```
...
cc-orch usage compare <a> <b>                   # cost / token breakdown across archives
cc-orch ui [--port N]                           # local web dashboard
cc-orch health                                  # config + state integrity checks
cc-orch clean [--force]                         # clear stale .harness/ state
cc-orch init [spec.md] | version | help
```

`cc-orch help` prints the same list from the router. Models per role are configurable in `src/orchestrator/infra/config.js`.
...
- [`commands.md`](./.claude/skills/cc-orch-operator/references/commands.md) — authoritative command / flag reference.
- [`spec-authoring.md`](./.claude/skills/cc-orch-operator/references/spec-authoring.md) — the hand-written spec contract (six sections, declared files, verification shapes).
- [`state-layout.md`](./.claude/skills/cc-orch-operator/references/state-layout.md) — on-disk state (`.harness/`, `queue/`, `archives/`, `refs/park/`).
- [`gotchas.md`](./.claude/skills/cc-orch-operator/references/gotchas.md) — sharp edges to know about.
