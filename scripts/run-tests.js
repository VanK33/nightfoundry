 ...
 *
 * Each test file is run via spawnSync('node', [testFile]).
 * The special `npm run test:hard-checks-pipeline-wiring` entry is run via
 * spawnSync('npm', ['run', 'test:hard-checks-pipeline-wiring']).
 *
...
// The special npm-run entry is represented as { npm: true, args: [...] }.
...
if (isMain) {
  ...
  for (const entry of TEST_FILES) {
    const { label, passed } = runEntry(entry);
    results.push({ label, passed });
    // No fail-fast: continue regardless of result.
  }
