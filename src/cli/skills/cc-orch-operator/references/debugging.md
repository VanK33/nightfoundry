...
- **Symptom:** after resolving a parked entry (`cc-orch park resolve <slug>` with `--requeue`, `--waive`, or `--reject`), either the active-run pointer or the parked run's `.harness/<runId>/` directory looks wrong afterward — the pointer still names a run that is gone, or a `cc-orch clean --runs` pass reaped (or preserved) a pointer in a way that seems inconsistent. Separately, a requeued (park-resume) entry's re-run seems to skip the full-suite baseline arm, or a preflight clean-tree check rejects WIP that the operator expects it to accept.
...
- **Act:**
  ...
  - **`cc-orch clean --runs` pointer reaping:** the decision is made from a single filesystem fact, and nothing else — whether `runHarnessDir(projectRoot, pointer.runId)` exists on disk. A pointer whose `runId` has no corresponding run directory on disk is orphaned and is reaped (removed). A pointer whose run directory still exists is left in place, untouched.
