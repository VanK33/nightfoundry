...
- **[references/brainstorm-method.md](references/brainstorm-method.md)** — the elicitation METHOD behind a spec that survives the gates: frame-first restatement, one-question-at-a-time with the five hardening categories, filesystem-grounded scope enumeration, gate-runnable acceptance evidence. Read before interviewing a user (or yourself) toward a new spec; pairs with spec-authoring.md (format).
