...
- **The reusable planner session rotates — extra `planner-reusable-N` logs are expected, not a leak.** The reusable planner session rotates at mission boundaries every `config.tokens.rotationMissionCount` missions, or sooner if its context reaches the 100k force-new threshold (with an 80k warn tier and a 120k alarm tier along the way). Each rotation swaps in a fresh session, so a run's session logs accumulate additional `planner-reusable-N` files with an incrementing numeric suffix — seeing several planner logs in one run reflects expected rotation, not a bug. Every rotation, warn, and alarm event is also appended to the warnings ledger under the `planner-rotation` category, so grep the ledger for `planner-rotation` to see the rotation history for a run.
