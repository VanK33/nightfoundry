/**
 * test-review-remediation-planner.js — Unit tests for Planner.remediateReviewFindings.
 *
 * No live Claude sessions are spawned — sessionManager.spawn is replaced
 * by a mock fixture following the test-assumption-remediation.js pattern.
 *
 * Run: node test/test-review-remediation-planner.js
 */
import assert from 'assert';
import { reviewRemediationSchema } from '../src/orchestrator/agents/_schemas.js';
...
const fixtureNewTasks = [
  {
    id: '001-002-003-001',
    description: 'Fix missing null check in executor.js',
    targetFiles: ['src/orchestrator/agents/executor.js'],
  },
  {
    id: '001-002-003-002',
    description: 'Add error handling in pipeline.js',
...
// Sample findings passed to remediateReviewFindings
const sampleFindings = [
  ...
  },
  {
    ...
    description: 'Error from spawn() is not caught when tokenTracker is absent',
  },
];
...
// ── TC1: spawn is called with jsonSchema equal to reviewRemediationSchema ──

await test('TC1: spawn is called with jsonSchema equal to reviewRemediationSchema', async () => {
  ...
  assert.deepStrictEqual(
    ...
    `spawn jsonSchema should equal reviewRemediationSchema.\nGot: ${JSON.stringify(calls[0].jsonSchema)}`,
  );
});
...
await test('TC3: return value has newTasks array matching the fixture', async () => {
  ...
  const result = await planner.remediateReviewFindings('001-002', sampleFindings, '/tmp/project');
...
await test('TC4: empty findings array produces a valid prompt (no crash)', async () => {
  ...
  let threw = false;
  try {
    result = await planner.remediateReviewFindings('001-002', [], '/tmp/project');
  } catch (err) {
    threw = true;
    assert.fail(`remediateReviewFindings threw with empty findings: ${err.message}`);
  }
...
// ── TC5: Session name includes milestoneId for log correlation ───────────────

await test('TC5: session name includes milestoneId for log correlation', async () => {
  const milestoneId = '002-003';
  const { mockSessionManager, getSpawnCalls } = makeMockSessionManager();
  const planner = new Planner(mockSessionManager, makeMockLogger(), null);

  await planner.remediateReviewFindings(milestoneId, sampleFindings, '/tmp/project');

  const calls = getSpawnCalls();
  assert.equal(calls.length, 1, 'Expected 1 spawn call');
  assert.ok(
    calls[0].name && calls[0].name.includes(milestoneId),
    `Session name should include milestoneId "${milestoneId}".\nActual name: "${calls[0].name}"`,
  );
});
